...
void apiBusCan::ActiveResponseAction(string cmd, string node, string args) {
	...
	case cm_OnTrack: {
		...
		break;
	case res_TestLink:{
		string call = (string) "OnResponse(" + "2" + ",'testCtrLink','')";
		this->runProcedure(call,node);
	}
		break;
	case res_Reset:{
		string call = (string) "OnResponse(" + "2" + ",'reset','')";
		this->runProcedure(call,node);
	}
		break;
	case res_GetFirmware:{
		string call = (string) "OnResponse(" + "2" + ",'getFirmware','"+args+"')";
		this->runProcedure(call,node);
	}
		break;
	case res_GetCFG:{
			string call = (string) "OnResponse(" + "2" + ",'getCFG','"+args+"')";
			this->runProcedure(call,node);
		}
			break;
	case res_SaveAndRestoreDisplay:
